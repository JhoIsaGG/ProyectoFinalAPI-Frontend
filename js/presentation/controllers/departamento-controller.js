/* Controlador de Catálogo: Departamentos */
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadItems() {
        try {
            ...
            allItems = await catalogRepository.getAll('departamentos');
            renderTable();
        } catch (error) {
            console.error(error);
            showGlobalError(`Error al cargar los departamentos: ${error.message}`);
        }
    }
    ...
    function openNewModal() {
        modalTitle.innerText = 'Nuevo Departamento';
        form.reset();
        inputId.value = '';
    ...
    function openEditModal(id) {
        modalTitle.innerText = `Editar Departamento #${id}`;
        form.reset();
        inputId.value = id;
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (isEditing) {
                await catalogRepository.update('departamentos', Number(id), payload);
            } else {
                await catalogRepository.create('departamentos', payload);
            }
            closeModal();
            await loadItems();
        } catch (error) {
            ...
            if (error.errors) {
                ...
                showModalError(error.message || 'Error al guardar el departamento.');
            }
        }
    });

    async function deleteItem(id) {
        if (confirm(`¿Está seguro de que desea eliminar lógicamente el departamento #${id}?`)) {
            try {
                const success = await catalogRepository.delete('departamentos', id, currentUser.id);
                if (success) {
                    await loadItems();
                }
            } catch (error) {
                console.error(error);
                showGlobalError(error.message || 'Error al eliminar el departamento.');
            }
        }
    }
