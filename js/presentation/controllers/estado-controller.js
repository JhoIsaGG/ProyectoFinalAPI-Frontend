/* Controlador de Catálogo: Estados de Ticket */
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadItems() {
        try {
            ...
            allItems = await catalogRepository.getAll('estados');
            renderTable();
        } catch (error) {
            console.error(error);
            showGlobalError(`Error al cargar los estados: ${error.message}`);
        }
    }
    ...
    function openNewModal() {
        modalTitle.innerText = 'Nuevo Estado';
        form.reset();
        inputId.value = '';
    ...
    function openEditModal(id) {
        modalTitle.innerText = `Editar Estado #${id}`;
        form.reset();
        inputId.value = id;
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (isEditing) {
                await catalogRepository.update('estados', Number(id), payload);
            } else {
                await catalogRepository.create('estados', payload);
            }
            closeModal();
            await loadItems();
        } catch (error) {
            ...
            if (error.errors) {
                ...
                showModalError(error.message || 'Error al guardar el estado.');
            }
        }
    });

    async function deleteItem(id) {
        if (confirm(`¿Está seguro de que desea eliminar el estado #${id}?`)) {
            try {
                const success = await catalogRepository.delete('estados', id, currentUser.id);
                if (success) {
                    await loadItems();
                }
            } catch (error) {
                console.error(error);
                showGlobalError(error.message || 'Error al eliminar el estado.');
            }
        }
    }
