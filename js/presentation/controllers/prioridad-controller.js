/* Controlador de Catálogo: Prioridades de Ticket */
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadItems() {
        try {
            ...
            allItems = await catalogRepository.getAll('prioridades');
            // Ordenar en base a la columna orden por defecto para mostrar coherencia
            allItems.sort((a, b) => Number(a.orden) - Number(b.orden));
            renderTable();
        } catch (error) {
            console.error(error);
            showGlobalError(`Error al cargar las prioridades: ${error.message}`);
        }
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (isEditing) {
                await catalogRepository.update('prioridades', Number(id), payload);
            } else {
                await catalogRepository.create('prioridades', payload);
            }
            closeModal();
            await loadItems();
        } catch (error) {
            ...
            if (error.errors) {
                ...
                showModalError(error.message || 'Error al guardar la prioridad.');
            }
        }
    });

    async function deleteItem(id) {
        if (confirm(`¿Está seguro de que desea eliminar la prioridad #${id}?`)) {
            try {
                const success = await catalogRepository.delete('prioridades', id, currentUser.id);
                if (success) {
                    await loadItems();
                }
            } catch (error) {
                console.error(error);
                showGlobalError(error.message || 'Error al eliminar la prioridad.');
            }
        }
    }
