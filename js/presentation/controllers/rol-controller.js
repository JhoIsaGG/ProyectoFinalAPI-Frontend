...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadItems() {
        try {
            ...
            allItems = await catalogRepository.getAll('roles');
            renderTable();
        } catch (error) {
            console.error(error);
            showGlobalError(`Error al cargar los roles: ${error.message}`);
        }
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (isEditing) {
                await catalogRepository.update('roles', Number(id), payload);
            } else {
                await catalogRepository.create('roles', payload);
            }
            closeModal();
            await loadItems();
        } catch (error) {
            ...
            if (error.errors) {
                ...
                showModalError(error.message || 'Error al guardar el rol.');
            }
        }
    });

    async function deleteItem(id) {
        if (confirm(`¿Está seguro de que desea eliminar lógicamente el rol #${id}?`)) {
            try {
                const success = await catalogRepository.delete('roles', id, currentUser.id);
                if (success) {
                    await loadItems();
                }
            } catch (error) {
                console.error(error);
                showGlobalError(error.message || 'Error al eliminar el rol.');
            }
        }
    }
